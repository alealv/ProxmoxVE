...
function update_container() {
  ...
  if [[ "$os" == "ubuntu" || "$os" == "debian" ]]; then
    ...
    if pct exec "$container" -- [ -e /usr/bin/update ]; then
      if pct exec "$container" -- grep -q "alealv/ProxmoxVE" /usr/bin/update; then
        ...
        pct exec "$container" -- bash -c "sed -i 's/tteck\\/Proxmox/alealv\\/ProxmoxVE/g' /usr/bin/update"
...
header_info
echo -e "${GN}The process is complete. The repositories have been switched to alealv/ProxmoxVE.${CL}\n"
